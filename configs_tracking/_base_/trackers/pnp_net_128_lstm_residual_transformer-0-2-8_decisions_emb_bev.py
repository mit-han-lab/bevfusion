...
model=dict(
        net=dict(
            MLPDetFalsePositive=[dict(type='LinearRes', n_in=256, n_out=256, norm='GN',ng=8),
                                 dict(type='Linear', in_features=256, out_features=1)],
            
            MLPTrackFalsePositive=[dict(type='LinearRes', n_in=256, n_out=256, norm='GN',ng=8),
                                   dict(type='Linear', in_features=256, out_features=1)],
            
            MLPTrackFalseNegative=[dict(type='LinearRes', n_in=256, n_out=256, norm='GN',ng=8),
                                   dict(type='Linear', in_features=256, out_features=1)],

            MLPDetNewborn=[dict(type='LinearRes', n_in=256, n_out=256, norm='GN',ng=8),
                            dict(type='Linear', in_features=256, out_features=1)],
